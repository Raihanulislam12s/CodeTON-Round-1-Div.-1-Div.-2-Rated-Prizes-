...
int main() {
	...
	while (T--) {
		...
		for (int i = 0; i < N; i++) {
			for (int j = 0; j < M; j++) {
				A_seg[{i, M+j}] = gcds[{i, j}];
			}
			for (int a = M-1; a; a--) {
				A_seg[{i, a}] = lcm(A_seg[{i, 2*a}], A_seg[{i, 2*a+1}]);
			}
			if (A_seg[{i, 1}] != A[i]) {
				A_alive[i] = false;
				A_queue.push_back(i);
		...
		for (int j = 0; j < M; j++) {
			for (int i = 0; i < N; i++) {
				B_seg[{j, N+i}] = gcds[{i, j}];
			}
			for (int a = N-1; a; a--) {
				B_seg[{j, a}] = lcm(B_seg[{j, 2*a}], B_seg[{j, 2*a+1}]);
			}
			if (B_seg[{j, 1}] != B[j]) {
				B_alive[j] = false;
				B_queue.push_back(j);
